Gamecoin Core version *0.15.0* is now available from:

  <https://download.gamecoin.org/gamecoin-0.15.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/gamecoin-dev>
...
installer (on Windows) or just copy over `/Applications/Gamecoin-Qt` (on Mac)
or `gamecoind`/`gamecoin-qt` (on Linux).
...
Gamecoin Core is extensively tested on multiple operating systems using
the Linux kernel, macOS 10.8+, and Windows Vista and later. Windows XP is not supported.

Gamecoin Core should also work on most other Unix-like systems but is not
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Gamecoin Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Gamecoin Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Gamecoin, either on the command line or in the Gamecoin config file.
...
Gamecoin Core 0.15.0 contains the following changes to the RPC interface and `gamecoin-cli` for multi-wallet:

* When running Gamecoin Core with a single wallet, there are **no** changes to the RPC interface or `gamecoin-cli`. All RPC calls and `gamecoin-cli` commands continue to work as before.
* When running Gamecoin Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `gamecoin-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Gamecoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:9332/wallet/wallet1.dat/`. `gamecoin-cli` commands should be run with a `-rpcwallet` option, for example `gamecoin-cli -rpcwallet=wallet1.dat getbalance`.
...
In previous versions of Gamecoin Core, a portion of each block could be reserved for transactions based on the age and value of UTXOs they spent. This concept (Coin Age Priority) is a policy choice by miners, and there are no consensus rules around the inclusion of Coin Age Priority transactions in blocks. In practice, only a few miners continue to use Coin Age Priority for transaction selection in blocks. Gamecoin Core 0.15 removes all remaining support for Coin Age Priority (See [PR 9602](https://github.com/bitcoin/bitcoin/pull/9602)). This has the following implications:
...
- When using Gamecoin Core in multi-wallet mode, RPC requests for wallet methods must specify
...
- #62005 `fa625b0` Document CWalletTx::mapValue entries and remove erase of nonexistent "version" entry (ryanofsky)
