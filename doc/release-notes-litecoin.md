Gamecoin Core version *0.15.1* is now available from:

  <https://download.gamecoin.org/gamecoin-0.15.1/>
...
A number of changes to the way Gamecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 games per kB (0.00001 GMC). This is to prepare for dropping the minimum transaction fee to 0.1 games per kB (0.0001 GMC) in the next release.
